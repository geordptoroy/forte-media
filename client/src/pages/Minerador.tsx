...
import { Search, Filter, Loader2, AlertCircle, Globe, LayoutGrid, Tag, Package, Clock } from "lucide-react";
...
export default function Minerador() {
  ...
  const [adType, setAdType] = useState<"ALL" | "POLITICAL_AND_ISSUE_ADS" | "NON_POLITICAL">("ALL");
  const [niche, setNiche] = useState("Todos");
  const [productType, setProductType] = useState("Todos");
  const [activeSince, setActiveSince] = useState("anytime");
  ...
  const { data: ads, isLoading, error, refetch } = trpc.ads.search.useQuery(
    { 
      searchTerms, 
      country, 
      adType: adType === "NON_POLITICAL" ? "ALL" : adType,
      niche,
      productType,
      activeSince
    },
    ...
    <DashboardLayout>
      <div className="flex flex-col space-y-8 max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          {ads && (
            <div className="flex items-center gap-4 px-4 py-2 bg-white/[0.03] border border-white/[0.06] rounded-lg">
        ...
        {/* Search & Filters Card */}
        <Card className="p-8 bg-black border-white/[0.06] rounded-xl shadow-2xl relative overflow-hidden group">
          <div className="absolute top-0 left-0 w-1 h-full bg-white/10 group-hover:bg-white/40 transition-colors" />
          
          <form onSubmit={handleSearch} className="space-y-8">
            {/* Primeira Linha: Busca e País */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-8 space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-12 bg-white/[0.03] border-white/[0.08] rounded-lg h-12 text-sm font-medium focus:border-white/20 focus:ring-0 transition-all placeholder:text-white/10"
                  />
                </div>
              </div>
              ...
              <div className="lg:col-span-4 space-y-2">
                ...
                <Select value={country} onValueChange={setCountry}>
                  <SelectTrigger className="bg-white/[0.03] border-white/[0.08] rounded-lg h-12 text-xs font-bold uppercase tracking-tighter focus:ring-0">
                    <div className="flex items-center gap-2">
                      <Globe className="w-3.5 h-3.5 text-white/20" />
                      <SelectValue placeholder="Selecionar País" />
                    </div>
                  </SelectTrigger>
                  <SelectContent className="bg-black border-white/[0.1] rounded-lg max-h-[300px]">
                    {COUNTRIES.map((c) => (
            ...
            {/* Segunda Linha: Nicho, Tipo e Tempo */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="space-y-2">
                ...
                <Select value={niche} onValueChange={setNiche}>
                  ...
                  <SelectContent className="bg-black border-white/[0.1] rounded-lg max-h-[300px]">
                    <SelectItem value="Todos" className="text-[10px] font-bold uppercase py-3">Todos os Nichos</SelectItem>
                    {NICHES.map((n) => (
                      <SelectItem key={n} value={n} className="text-[10px] font-bold uppercase py-3">
                        {n}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Select value={productType} onValueChange={setProductType}>
                  ...
                  <SelectContent className="bg-black border-white/[0.1] rounded-lg">
                    {PRODUCT_TYPES.map((t) => (
                      <SelectItem key={t} value={t} className="text-[10px] font-bold uppercase py-3">
                        {t}
                      </SelectItem>
                    ))}
                  </SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Select value={activeSince} onValueChange={setActiveSince}>
                  <SelectTrigger className="bg-white/[0.03] border-white/[0.08] rounded-lg h-12 text-xs font-bold uppercase tracking-tighter focus:ring-0">
                    <div className="flex items-center gap-2">
                  ...
                  <SelectContent className="bg-black border-white/[0.1] rounded-lg">
                    <SelectItem value="anytime" className="text-[10px] font-bold uppercase py-3">Qualquer tempo</SelectItem>
                    <SelectItem value="7d" className="text-[10px] font-bold uppercase py-3">Últimos 7 dias</SelectItem>
                    <SelectItem value="30d" className="text-[10px] font-bold uppercase py-3">Últimos 30 dias</SelectItem>
                    <SelectItem value="90d" className="text-[10px] font-bold uppercase py-3">Últimos 90 dias</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select value={adType} onValueChange={(v: any) => setAdType(v)}>
                  ...
                  <SelectContent className="bg-black border-white/[0.1] rounded-lg">
                    <SelectItem value="ALL" className="text-[10px] font-bold uppercase py-3">Todos</SelectItem>
                    <SelectItem value="NON_POLITICAL" className="text-[10px] font-bold uppercase py-3">Apenas Não Políticos</SelectItem>
                    <SelectItem value="POLITICAL_AND_ISSUE_ADS" className="text-[10px] font-bold uppercase py-3">Apenas Políticos</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-white/[0.04]">
              <p className="text-[9px] font-bold text-white/20 uppercase tracking-widest hidden md:block">
                Dica: Filtre por "Encapsulado" e "Últimos 30 dias" para encontrar tendências de escala.
              </p>
              <Button 
                type="submit" 
                disabled={isLoading}
                className="w-full md:w-auto h-12 px-12 bg-white text-black hover:bg-white/90 rounded-lg font-black uppercase tracking-[0.2em] text-xs transition-all active:scale-95"
              >
                {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 space-y-8">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-[10px] text-white/30 font-bold uppercase tracking-widest">
                Extraindo dados demográficos, nichos e métricas de alcance...
              </p>
            </div>
          </div>
        )}
        ...
        {/* Error State */}
        {error && (
          <Card className="p-8 border-red-500/20 bg-red-500/5 rounded-xl relative overflow-hidden">
            ...
            <div className="flex items-start space-x-6">
              ...
              <div className="space-y-1">
                ...
                <Button 
                  ...
                  size="sm" 
                  onClick={() => refetch()}
                  className="mt-4 border-red-500/20 text-red-500 hover:bg-red-500/10 rounded-lg text-[9px] font-black uppercase"
                >
                  Tentar Novamente
                </Button>
              </div>
        ...
        {hasSearched && !isLoading && ads && ads.length === 0 && (
          <div className="flex flex-col items-center justify-center py-32 border border-dashed border-white/10 rounded-xl space-y-4">
